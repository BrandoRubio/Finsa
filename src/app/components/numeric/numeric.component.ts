...
export class NumericComponent implements OnInit {
  ...
  GetSensorValue() {
    ...
    this.api.GetRequestRender(`sensorsLatest/?sensors=${sensorIDsString}`, false).then((response: any) => {
      this.widgetData.sensors = this.widgetData.sensors.map((sensor: any) => {
        const apiSensor = response.data.find((item: any) => item.sensor_id === sensor.sensor_id);
        if (apiSensor) {
          return {
            ...sensor,
      ...
      /*this.lastValue = response.data[0].value

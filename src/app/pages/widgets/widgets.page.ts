...
import { ApexAxisChartSeries, ApexChart, ApexDataLabels, ApexStroke, ApexTooltip, ApexXAxis, ApexYAxis } from 'apexcharts';
...
export class WidgetsPage implements OnInit {
  ...
  constructor(
    ...
    addIcons({
      menuOutline, addCircleOutline, trashOutline, checkmark, listOutline, gridOutline, pieChartOutline, albumsOutline, timeOutline,
      calendarOutline, pencilOutline, hardwareChipOutline, thermometerOutline, flashOutline, pulseOutline, waterOutline, expandOutline
    });
  ...
  ngOnInit() {
    ...
    this.dashId = +this.route.snapshot.params['dashboard_id'];
  ...
  GetWidgets() {
    this.api.GetRequestRender('dashboards/' + this.dashId + '/widgets').then((response: any) => {
      ...
      /*
            ...
            this.dashboardName = raw.dashboard_name;
            this.dashboardDescription = raw.dashboard_description;
            this.plantName = raw.plant_name;
            this.parkName = raw.park_name;*/

      // Mapeo de widgets igual que tu proyecto anterior
      this.widgets = raw.widgets.map((item: any, index: number) => ({
      ...
      this.api.GetRequestRender('machinesAndSensorsByOrganizations?organizations=' + this.plantId, false).then((response: any) => {

...
import {
  ...
  pieChartOutline, albumsOutline, timeOutline, calendarOutline, pencilOutline, personAddOutline, cubeOutline, personRemoveOutline, closeOutline, businessOutline, trashOutline
} from 'ionicons/icons';

@Component({
  selector: 'app-parks-users',
  templateUrl: './parks-users.page.html',
  styleUrls: ['./parks-users.page.scss'],
  standalone: true,
...
export class ParksUsersPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.parkName = this.router.getCurrentNavigation()?.extras?.state?.['park_name'] ?? '';
    this.GetUsers();
  }
  ...
  // ─── API ─────────────────────────────────────────────────────
  GetUsers() {
    this.api.GetRequestRender('parks/' + this.parkId + '/users').then((response: any) => {
  ...
  SaveUser() {
    if (this.selectedUser) {
      // Editar rol en el parque
      this.api.PutRequestRender(
        'parks/' + this.parkId + '/users/' + this.selectedUser.user_id,
        { role: this.form.role }
      ).then((response: any) => {
      ...
      // Asignar usuario existente al parque
      this.api.PostRequestRender('parks/' + this.parkId + '/users', {
      ...
      // Crear usuario nuevo y asignarlo al parque
      const body = {
        name: this.form.name,
  ...
  RevokeAccess(user: any) {
    const confirmed = confirm(`¿Quitar acceso de ${user.name} a este parque?`);
    if (!confirmed) return;
    this.api.DeleteRequestRender('parks/' + this.parkId + '/users/' + user.user_id)
      .then((response: any) => {
  ...
  OpenModal(user: any) {
    ...
    this.userSearch = '';
    this.form = user
      ? { name: user.name, email: user.email, password: '', role: user.park_role || 'viewer', existing_user_id: user.user_id }
